...
import esprit.rt.dao.UtilisateurDAO;
import esprit.rt.entities.Client;
import javax.swing.JOptionPane;
import org.eclipse.swt.widgets.MessageBox;
import esprit.rt.gui.AuthentificationClientRest;
import static esprit.rt.gui.AuthentificationClientRest.access_token;
import esprit.rt.gui.FlatosDialogos;
import esprit.rt.gui.MenuConsultationRestaurant;
...
//    rawJsonResponse();
  ...
  public void fetchObject() {
    ...
    out.println("Sexe : " + user.getGender());
    AuthentificationClientRest.nom=user.getName();
    
    //out.println("Page likes: " + page.getLikes());
      UtilisateurDAO utilisateurDAO = new UtilisateurDAO();
      int res = utilisateurDAO.findUtilisateurtByEmail(user.getEmail());
      if (res == -1) 
      {
      ...
      String birdhday = user.getBirthday();
      
      String mois = birdhday.substring(0,2);
      String jour = birdhday.substring(3,5);
      String annee = birdhday.substring(6,10);
      
      String dateconcat = annee+"-"+mois+"-"+jour;
      
      Date date1 = Date.valueOf(dateconcat);
      //System.out.println(dateconcat);
      //System.out.println("moi :"+mois+"jour"+jour+"annee"+annee);
       
      ClientDAO clientDAO = new ClientDAO();
      clientDAO.insertClient(user.getName(), user.getFirstName(), user.getEmail(),"",date1,0, "", "", "",sexe, "", "",""); 
          new FlatosDialogos(SessionRestoTunisie.auth, "Vous étes inscris! Vous pouvez maintenant connecter en utilisant votre compte facebook! ", 50f, false).setVisible(true);
      }
      else
      {
         SessionRestoTunisie.setId(utilisateurDAO.findUtilisateurtByEmail(user.getEmail()));
         new MenuConsultationRestaurant().setVisible(true);
         SessionRestoTunisie.auth.dispose();
          // PUTT CLIENT CODE HERE MAN!
      }
